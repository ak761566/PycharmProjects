...
def start_completeness_check_on_audit_site(xsls_file_path, sheet_name, result_text, window, provider, log_folder_path, html_folder_path ):
    ...
    try:
        ...
        for (index, row) in data_frame.iterrows():
            ...
            result_text.insert(END, f"Processing {item_count}/{len(data_frame)} row.\n")
            window.update()
        ...
        if len(Audit_Findings) > 0:
            ...
            if len(title_not_found_on_audit) > 0:
                result_text.insert(END, f"----------------List of Titles not found on the Audit Site under the provider {provider.upper()} ----- \n\n")
